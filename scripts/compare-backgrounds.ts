...
console.log("-".repeat(80));
console.log("Step | OLD HEX   | NEW HEX   | OLD L   | NEW L   | L Diff   | Changed?");
console.log("-".repeat(80));
...
if (significantChanges > 0) {
  ...
  console.log("This is because generateRadixColors uses transposeProgressionStart()");
  console.log("which adjusts the lightness curve based on the background color.");
} else {
  console.log();
  console.log("✅ The background color does NOT significantly affect scale generation.");
}
...
for (let i = 0; i < 4; i++) {
  ...
  console.log(`  Step ${i + 1}: OLD=${oldAlpha} | NEW=${newAlpha} | ${changed}`);
}
