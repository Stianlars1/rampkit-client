...
const inputHex = "#3B82F6"; // The blue from the screenshot
...
for (let i = 0; i < 4; i++) {
  ...
  const chromaAssessment = C < 0.01 ? "✅ Low" : C < 0.02 ? "⚠️ Medium" : "❌ High";
...
lightScale.accentScale.forEach((hex, i) => {
  const color = new Color(hex).to("oklch");
  console.log(`  Step ${(i + 1).toString().padStart(2)}: ${hex} (L=${color.coords[0].toFixed(3)}, C=${color.coords[1].toFixed(3)})`);
});
...
console.log(`Light bg OKLCH: L=${lightBg.coords[0].toFixed(4)}, C=${lightBg.coords[1].toFixed(4)}, H=${lightBg.coords[2]?.toFixed(1) || 'N/A'}`);
console.log(`Dark bg OKLCH: L=${darkBg.coords[0].toFixed(4)}, C=${darkBg.coords[1].toFixed(4)}, H=${darkBg.coords[2]?.toFixed(1) || 'N/A'}`);

console.log();
console.log("=".repeat(80));
console.log("RADIX REFERENCE (approximate expected values for blue)");
console.log("=".repeat(80));
console.log("Step 1 (Light): Should be ~#fdfdfe or similar (very light, minimal tint)");
